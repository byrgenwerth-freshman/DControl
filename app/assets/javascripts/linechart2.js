...
//Modified: Matt Owens and Kelsey Leftwich
//
...
$(document).ready(function(){
	...
	$.get("/d_control/bargraph", function(msg){
		...
		tmpArray = getUnique(tmpArray); //returns array of unique datacenter group ids
		
		//add option of "All Selected Data" to drop-down menu
		$("select[id='Groupings']").append('<option value="orig">'
						   + 'All Selected Data</option>');

		//for every element in tmpArray, add element as option in drop-down menu
		...
		//Get dates from all data items ("gathered" value)
		tmpArray = [];
		for(var i = 0; i < data.length; i++){
		...
		//add option of "Select All Dates" to drop-down menu
...
function makeGraph(object, compItem)
{
	...
	x.domain(d3.extent(object, function(d) { return d.dns_name; }));
